...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="fas fa-edit me-2"></i>تعديل الفاتورة</h2>
    <div>
                            ...
                            @if($hasItems)
                                @foreach($items as $index => $item)
                                    @php
                                        ...
                                        $isHoursType = isset($itemData->quantity) && $itemData->quantity > 0 && isset($itemData->unit_price) && $itemData->unit_price > 0;
                                    @endphp
                                    
                                    @if($isHoursType && $itemData->quantity != 1)
                                        <div class="invoice-item row mb-3 d-none" data-entry-type="amount">
                                            <div class="col-md-6">
                                                <label class="form-label">الوصف</label>
                                                <input type="text" class="form-control" 
                                                       name="items[{{ $index }}][description]" 
                                                       value="{{ old("items.$index.description", $itemData->description ?? '') }}"
                                            ...
                                            <div class="col-md-5">
                                                ...
                                                <input type="number" class="form-control item-amount" 
                                                       ...
                                                       value="{{ old("items.$index.amount", ($itemData->quantity * $itemData->unit_price)) }}" 
                                                       min="0" step="0.01" required>
                                                <input type="hidden" class="form-control quantity" name="items[{{ $index }}][quantity]" value="1">
                                                <input type="hidden" class="form-control unit-price" name="items[{{ $index }}][unit_price]" value="{{ old("items.$index.unit_price", $itemData->quantity * $itemData->unit_price) }}">
                                            </div>
                                            <div class="col-md-1">
                                                <label class="form-label">&nbsp;</label>
                                                <button type="button" class="btn btn-outline-danger btn-sm w-100 remove-item" {{ count($items) == 1 ? 'disabled' : '' }}>
                                                    <i class="fas fa-trash"></i>
                                                </button>
                                            </div>
                                        </div>
                                        
                                        <div class="invoice-item row mb-3" data-entry-type="hours">
                                            <div class="col-md-4">
                                                <label class="form-label">الوصف</label>
                                                <input type="text" class="form-control" 
                                                       name="items[{{ $index }}][description]" 
                                            ...
                                            <div class="col-md-2">
                                                ...
                                                <input type="number" class="form-control item-hours" 
                                                       name="items[{{ $index }}][hours]" 
                                                       value="{{ old("items.$index.hours", $itemData->quantity ?? 0) }}" 
                                                       min="0" step="0.01" required>
                                            </div>
                                            <div class="col-md-3">
                                                <label class="form-label">سعر الساعة <span class="text-danger">*</span></label>
                                                <input type="number" class="form-control item-hourly-rate" 
                                                       name="items[{{ $index }}][hourly_rate]" 
                                                       value="{{ old("items.$index.hourly_rate", $itemData->unit_price ?? 0) }}" 
                                                       min="0" step="0.01" required>
                                            </div>
                                            <div class="col-md-2">
                                                <label class="form-label">المجموع</label>
                                                <input type="text" class="form-control item-total" readonly value="{{ number_format(($itemData->quantity ?? 0) * ($itemData->unit_price ?? 0), 2) }}">
                                                <input type="hidden" class="form-control quantity" name="items[{{ $index }}][quantity]" value="{{ old("items.$index.quantity", $itemData->quantity ?? 1) }}">
                                                <input type="hidden" class="form-control unit-price" name="items[{{ $index }}][unit_price]" value="{{ old("items.$index.unit_price", $itemData->unit_price ?? 0) }}">
                                            </div>
                                            <div class="col-md-1">
                                                <label class="form-label">&nbsp;</label>
                                                <button type="button" class="btn btn-outline-danger btn-sm w-100 remove-item" {{ count($items) == 1 ? 'disabled' : '' }}>
                                    ...
                                    @else
                                        <div class="invoice-item row mb-3" data-entry-type="amount">
                                            ...
                                            <div class="col-md-5">
                                                ...
                                                <input type="number" class="form-control item-amount" 
                                                       name="items[{{ $index }}][amount]" 
                                                       value="{{ old("items.$index.amount", $itemData->total ?? 0) }}" 
                                                       min="0" step="0.01" required>
                                                <input type="hidden" class="form-control quantity" name="items[{{ $index }}][quantity]" value="1">
                                                <input type="hidden" class="form-control unit-price" name="items[{{ $index }}][unit_price]" value="{{ old("items.$index.unit_price", $itemData->total ?? 0) }}">
                                            </div>
                                            <div class="col-md-1">
                                                <label class="form-label">&nbsp;</label>
                                                <button type="button" class="btn btn-outline-danger btn-sm w-100 remove-item" {{ count($items) == 1 ? 'disabled' : '' }}>
                                        ...
                                        <div class="invoice-item row mb-3 d-none" data-entry-type="hours">
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <input type="number" class="form-control item-hourly-rate" 
                                                       name="items[{{ $index }}][hourly_rate]" 
                                                       value="{{ old("items.$index.hourly_rate", '0') }}" 
                                                       min="0" step="0.01">
                                            </div>
                                            <div class="col-md-2">
                                                <label class="form-label">المجموع</label>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <textarea class="form-control @error('terms') is-invalid @enderror" 
                                      id="terms" name="terms" rows="3">{{ old('terms', $invoice->terms) }}</textarea>
                            @error('terms')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="alert alert-info" id="calculation-preview">
                        ...
                        <div class="row">
                            <div class="col-md-12 text-center">
                                <p class="mb-0"><strong>المجموع الإجمالي:</strong> <span id="total-amount" class="fs-4 text-primary">{{ number_format($invoice->total_amount, 2) }}</span> ج.م</p>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let itemIndex = {{ $hasItems ? count($items) : 1 }};
        ...
        if (entryType === 'amount') {
            ...
            if (amountInput) {
                amountInput.addEventListener('input', function() {
                    ...
                    if (unitPriceInput) unitPriceInput.value = amount;
                    calculateInvoiceTotal();
                });
            }
        } else {
            ...
            function calculateHoursTotal() {
                ...
                if (quantityInput) quantityInput.value = hours;
                if (unitPriceInput) unitPriceInput.value = hourlyRate;
                calculateInvoiceTotal();
            }
            
            if (hoursInput) hoursInput.addEventListener('input', calculateHoursTotal);
            if (hourlyRateInput) hourlyRateInput.addEventListener('input', calculateHoursTotal);
        }
    ...
    const activeEntryType = document.querySelector('input[name="entry_type"]:checked').value;
    document.querySelectorAll('.invoice-item').forEach(item => {
        if (item.dataset.entryType === activeEntryType) {
            if (activeEntryType === 'amount') {
                const amountInput = item.querySelector('.item-amount');
